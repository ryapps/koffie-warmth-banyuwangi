...
export const AdminMarquee = () => {
  const { settings, updateSettings, loadItems, saveItemsToSupabase } = useMarqueeStore();
  ...
  useEffect(() => {
    loadItems().then(() => {
      setItems(useMarqueeStore.getState().settings.items);
    });
  }, [loadItems]);
  ...
  const handleSave = async () => {
    ...
    try {
      await saveItemsToSupabase(filteredItems);
      updateSettings({
        items: filteredItems,
        speed,
      ...
      toast.success("Pengaturan Marquee berhasil disimpan ke Supabase!");
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Gagal menyimpan marquee";
      toast.error(msg);
    } finally {
